...
import { percent, topicCountOf } from "../../shared/progress.js";
import { avatarUrlOf, MIN_PASSWORD, profileOf, STAFF_FIELDS, staffDepartments } from "../../shared/profile.js";
 ...
 */
const canDeliverIn = (departmentId) => ({
    isActive: true,
    OR: [
 ...
 */
const readDepartmentIds = async (body, role, scope) => {
    const departmentIds = role === Role.HOD
        ? [...new Set((body.departmentIds || []).filter(Boolean))]
        : [body.departmentId].filter(Boolean);
    if (!departmentIds.length)
        throw fail(400, role === Role.HOD ? "Select at least one department" : "Department is required");
    if (scope && departmentIds.some(id => !inScope(scope, id)))
        throw fail(404, "Department not found");
    if (await prisma.department.count({ where: { id: { in: departmentIds } } }) !== departmentIds.length)
        throw fail(400, "One or more of those departments do not exist");
 ...
 */
const coursesOf = async (studentId) => {
...
router.get("/departments", allow(...MANAGERS), asyncHandler(async (req, res) => {
     ...
     */
    const instructorsOf = (departmentId) => deliverers
 ...
 */
router.patch("/staff/:id", allow(...MANAGERS), asyncHandler(async (req, res) => {
    ...
    if (nextRole !== target.role && scope)
        return res.status(403).json({ message: "Only an administrator can change a staff member's role" });
    const data = {};
    ...
    const departmentIds = submitted
        ...
        : target.role === Role.HOD
            ...
            : [target.departmentId].filter((id) => !!id);
    ...
    if (!departmentIds.length)
        return res.status(400).json({ message: nextRole === Role.HOD ? "Select at least one department" : "Department is required" });
 ...
 */
router.patch("/staff/:id/password", allow(...MANAGERS), asyncHandler(async (req, res) => {
    ...
    if (newPassword.length < MIN_PASSWORD)
        return res.status(400).json({ message: `The new password must be at least ${MIN_PASSWORD} characters` });
 ...
 */
router.get("/staff/:id", allow(...MANAGERS), asyncHandler(async (req, res) => {
    const target = await findManageableStaff(req, {});
    if (!target)
        return res.status(404).json({ message: "Account not found" });
 ...
 */
const findManageableStudent = async (req, where = {}) => {
    ...
    if (!target)
        return null;
    const scope = await scopeOf(req.user);
    if (scope && !target.memberOf.some(m => scope.includes(m.departmentId)))
        return null;
    return target;
};
/**
 ...
 */
const studentStatus = (cohorts) => !cohorts.length ? "NOT_ENROLLED" : cohorts.every(c => c.completedAt) ? "COMPLETED" : "IN_PROGRESS";
 ...
 */
router.get("/students", allow(...MANAGERS), asyncHandler(async (req, res) => {
    const scope = await scopeOf(req.user);
 ...
 */
router.get("/students/:id", allow(...MANAGERS), asyncHandler(async (req, res) => {
 ...
 */
router.delete("/students/:id", allow(...MANAGERS), asyncHandler(async (req, res) => {
...
router.patch("/students/:id/reactivate", allow(...MANAGERS), asyncHandler(async (req, res) => {
    const target = await findManageableStudent(req);
    if (!target)
        return res.status(404).json({ message: "Student not found" });
    if (target.isActive)
 ...
 */
router.patch("/students/:id/password", allow(...MANAGERS), asyncHandler(async (req, res) => {
...
router.patch("/cohorts/:id/instructor", allow(...MANAGERS), asyncHandler(async (req, res) => {
    ...
    if (!instructor)
        return res.status(400).json({ message: "Choose an active instructor from this department, or one of its heads" });
 ...
 */
router.get("/instructor/cohorts", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
...
router.post("/instructor/cohorts", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
    ...
    const candidates = user.role === Role.HOD
        ...
        : [user.departmentId].filter((id) => !!id);
    if (!candidates.length)
        return res.status(400).json({ message: user.role === Role.HOD ? "You do not head any department yet" : "Instructor has no assigned department" });
    const departmentId = req.body.departmentId ? String(req.body.departmentId) : candidates.length === 1 ? candidates[0] : null;
    if (!departmentId)
        return res.status(400).json({ message: "Choose which department this cohort belongs to" });
    ...
    if (!candidates.includes(departmentId))
        return res.status(404).json({ message: "Department not found" });
    const name = req.body.name?.trim();
    if (!name)
...
router.post("/instructor/cohorts/:cohortId/students", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
...
router.put("/instructor/cohorts/:cohortId/progress/:itemId", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
 ...
 */
router.patch("/instructor/cohorts/:cohortId/complete", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
...
router.patch("/instructor/cohorts/:cohortId/reopen", allow(Role.INSTRUCTOR, Role.HOD), asyncHandler(async (req, res) => {
    const cohort = await prisma.cohort.findFirst({ where: { id: String(req.params.cohortId), instructorId: req.user.id } });
    if (!cohort)
        return res.status(404).json({ message: "Cohort not found" });
    if (!cohort.completedAt)
