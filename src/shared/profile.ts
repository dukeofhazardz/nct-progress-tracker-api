 ...
 */
export const staffDepartments = (user: { role: Role; department: { id: string; name: string } | null; memberOf: { department: { id: string; name: string } }[] }) =>
  user.role === Role.HOD
 ...
 */
export const readAvatarUpload = (value: unknown) => {
  if (typeof value !== "string" || !value.trim()) throw badRequest("No image was uploaded");
  ...
  if (bytes.length > AVATAR_LIMIT) throw badRequest(`That picture is ${Math.round(bytes.length / 1024)} KB; the limit is ${AVATAR_LIMIT / 1024} KB`);

  return { bytes, mimeType: `image/${subtype}`, extension: EXTENSIONS[subtype as keyof typeof EXTENSIONS] };
};
 ...
 */
export const avatarUrlOf = (avatarPath: string | null) => (avatarPath ? publicUrl(avatarPath) : null);
 ...
 */
export const profileOf = async (userId: string) => {
