...
router.put("/departments/:id/curriculum", allow(...MANAGERS), asyncHandler(async (req: AuthRequest, res) => {
  ...
  await prisma.$transaction([prisma.curriculumItem.deleteMany({ where: { departmentId } }), ...items.map((x: any, position: number) => prisma.curriculumItem.create({ data: { departmentId, title: x.title.trim(), description: x.description, position } }))]);
