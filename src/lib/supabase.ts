...
export const publicUrl = (path: string) =>
  isStorageConfigured ? `${origin}/storage/v1/object/public/${BUCKET}/${path}` : null;
 ...
 */
export const putObject = async (path: string, bytes: Buffer, mimeType: string) => {
  if (!isStorageConfigured) throw fail(503, "Profile pictures are not configured on this server");
