...
router.patch("/password", asyncHandler(async (req: AuthRequest, res) => {
  const currentPassword = String(req.body.currentPassword ?? "");
  const newPassword = String(req.body.newPassword ?? "");
  if (!currentPassword || !newPassword) return res.status(400).json({ message: "Enter your current password and a new one" });
  if (newPassword.length < MIN_PASSWORD) return res.status(400).json({ message: `Your new password must be at least ${MIN_PASSWORD} characters` });
  ...
  if (!await bcrypt.compare(currentPassword, user.password)) return res.status(400).json({ message: "That is not your current password" });
  if (await bcrypt.compare(newPassword, user.password)) return res.status(400).json({ message: "Your new password must be different from your current one" });
 ...
 */
router.put("/avatar", asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (before?.avatarPath) await deleteObject(before.avatarPath);

  res.json({ avatarUrl: avatarUrlOf(path) });
}));

router.delete("/avatar", asyncHandler(async (req: AuthRequest, res) => {
  const before = await prisma.user.findUnique({ where: { id: req.user!.id }, select: { avatarPath: true } });
