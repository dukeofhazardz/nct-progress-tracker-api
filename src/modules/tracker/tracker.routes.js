...
router.put("/departments/:id/curriculum", allow(Role.ADMIN), asyncHandler(async (req, res) => {
    ...
    await prisma.$transaction([prisma.curriculumItem.deleteMany({ where: { departmentId: req.params.id } }), ...items.map((x, position) => prisma.curriculumItem.create({ data: { departmentId: req.params.id, title: x.title.trim(), description: x.description, position } }))]);
...
router.post("/instructors", allow(Role.ADMIN), asyncHandler(async (req, res) => {
    ...
    if (existing && existing.role !== Role.INSTRUCTOR)
...
router.patch("/instructors/:id/reactivate", allow(Role.ADMIN), asyncHandler(async (req, res) => {
    ...
    if (instructor.isActive)
        return res.status(409).json({ message: "That instructor is already active" });
...
router.post("/instructor/cohorts/:cohortId/students", allow(Role.INSTRUCTOR), asyncHandler(async (req, res) => {
    ...
    if (student.departmentId && student.departmentId !== cohort.departmentId)
        return res.status(400).json({ message: "Student belongs to another department" });
    if (!student.departmentId)
        await prisma.user.update({ where: { id: student.id }, data: { departmentId: cohort.departmentId } });
